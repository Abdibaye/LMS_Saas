...
export const courseSchema = z.object({
  ...
  slug: z.string().min(0, 'Slug is required').max(100, 'Slug must be less than 100 characters'),
  fileKey: z.string().min(0, 'File key is required'),
  level:courseLevelSchema,
  price: z.coerce.number().min(0, 'Price must be a positive number'),
  duration: z.coerce.number().min(1, 'Duration is required').max(500, 'Duration must be less than 500 characters'),
